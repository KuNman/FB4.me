...
EOT
    ...
    protected function executeRoleCommand(UserManipulator $manipulator, OutputInterface $output, $username, $super, $role)
    {
        if ($super) {
            ...
            $output->writeln(sprintf('User.php "%s" has been promoted as a super administrator. This change will not apply until the user logs out and back in again.', $username));
        } else {
            if ($manipulator->addRole($username, $role)) {
                ...
                $output->writeln(sprintf('User.php "%s" did already have "%s" role.', $username, $role));
            }
        }
    }
}
