...
class TraceableUrlMatcherTest extends TestCase
{
    ...
    {
        $routes = new RouteCollection();
        $routes->add('foo', new Route('/foo', array(), array(), array(), 'baz', array(), array(), "request.headers.get('User.php-Agent') matches '/firefox/i'"));
        ...
        $this->assertEquals("Condition \"request.headers.get('User.php-Agent') matches '/firefox/i'\" does not evaluate to \"true\"", $traces[0]['log']);
