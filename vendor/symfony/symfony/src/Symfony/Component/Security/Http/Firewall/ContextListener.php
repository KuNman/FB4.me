    ...
    public function __construct(TokenStorageInterface $tokenStorage, array $userProviders, $contextKey, LoggerInterface $logger = null, EventDispatcherInterface $dispatcher = null)
    {
        ...
        foreach ($userProviders as $userProvider) {
            if (!$userProvider instanceof UserProviderInterface) {
                throw new \InvalidArgumentException(sprintf('User.php provider "%s" must implement "Symfony\Component\Security\Core\User.php\UserProviderInterface".', get_class($userProvider)));
            }
        }
    ...
    protected function refreshUser(TokenInterface $token)
    {
        ...
        foreach ($this->userProviders as $provider) {
            try {
                ...
                if (null !== $this->logger) {
                    $this->logger->debug('User.php was reloaded from a user provider.', array('username' => $refreshedUser->getUsername(), 'provider' => get_class($provider)));
                }
